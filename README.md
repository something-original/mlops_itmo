...
| Модель | Кол-во параметров | Кол-во эпох | Размер тренировочного датасета | ExactMatch (тестовый датасет)| PartialMatch (тестовый датасет)| TrainLoss | ValLoss |
|--------|-------------------|-------------|-----------------|------------|--------------|-----------|---------|
...
Результаты тестирования путем [запросов к API](#11-api-сервис) подтвердили результаты, полученные во время обучения и валидации модели. Модель Master имеет достаточно большое время инференса (от нескольких десятков секунд до нескольких минут), в то время как инференс модели Parseq составляет всего несколько секунд. Однако, Master способен распознать текст, хоть и с невысокой точностью, в то время как Parseq просто подставляет случайные слова из тренировочной выборки.
...
API сервис реализован с использованием FastAPI и предоставляет следующие эндпоинты:

### Эндпоинты

1. `POST /compare` - сравнение двух документов
   - Принимает два файла (PDF или изображения)
   - Возвращает распознанный текст и метрики сравнения
   - Использует кэширование документов для оптимизации

2. `GET /metrics` - получение метрик производительности
   - Возвращает агрегированные метрики для текущей модели
   - Включает время инференса и точность

3. `GET /health` - проверка работоспособности сервиса

4. `DELETE /cache` - очистка закэшированных документов

### Конфигурация

Конфигурация API сервиса находится в файле `app/config.yaml`:
- Выбор модели (master/parseq)
- Настройки API (хост, порт)
...
Скрипт для тестирования API с тестовыми файлами:
```
cd app
python src\test_api.py sample_data\sample.jpg sample_data\sample_modified.jpg
```

Результаты тестирования сохраняются в файл `test_results.json` и включают:
- Время запроса
- Время инференса
- Отношение числа совпадающих символов к длине первой последовательности (оригинала)

### Метрики

API сервис собирает следующие метрики:
- Время инференса модели
- Точность распознавания текста
- Время обработки запроса
- Количество обработанных документов

Метрики хранятся в MongoDB и доступны через эндпоинт `/metrics`.

Выводы и анализ см. в [пункте 7](#7-результаты)

## 12. Мониторинг

В качестве инструмента для мониторинга выбраны Prometheus и Grafana. Их конфигурацию можно посмотреть в папке `monitoring`. Отслеживались такие показатели, как наличие/отсутствие распознавания (0, если текст не распознан, но есть на изображении), время инференса, время предсказания (от получения запроса до отправки ответа), количество запросов и тд. Полный список можно посмотреть в ```app/src/metrics.py```.

Для оповещения о выходе значений метрик из заданных пределов создан телеграм-бот @mlops_user_bot.

Примеры:

График времени инференса в Grafana:\
![График времени инференса в Grafana](screenshots/grafana_inference_time.png)

График распознавания текста в Grafana:\
![График распознавания текста в Grafana](screenshots/grafana_text_recognized.png)

График времени инференса в Prometheus:\
![График времени инференса в Prometheus](screenshots/prometheus_inference_time.png)

Оповещение в телеграм:\
![Оповещение в телеграм](screenshots/tg_inference_time_alert.png)

Метрики из эндпоинта Prometheus:\
![Метрики из эндпоинта Prometheus](screenshots/prometheus_metrics_endpoint.png)
